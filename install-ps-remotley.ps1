...
$session = New-PSSession -ComputerName cl1 -ConfigurationName PowerShell.7.4.7
Copy-Item -Path "C:\install\PowerShell-7.4.7-win-x64.msi" -Destination "C:\install" -ToSession $session
Invoke-Command -Session $session -ScriptBlock {
    Start-Process "msiexec.exe" -ArgumentList "/i C:\install\PowerShell-7.4.7-win-x64.msi /quiet /norestart" -Wait
}
